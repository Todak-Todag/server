...
사용자가 16번 재매칭을 끝내 시도하지 않으면 그 케어플랜은 `CarePlanCompleted`가 영원히 발행되지 않는 문제가 있었다(해소 경로가 재매칭 성공뿐이었기 때문). 아래 **보정 스윕**이 활동 기간이 끝난 케어플랜을 주기적으로 찾아 발행하고, 방치된 실패 이력은 `EXPIRED`로 종결시켜 이 한계를 해소한다.
