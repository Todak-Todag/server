...
>
> ⚠️ 확인 필요: **409(`이미 삭제 됨`) 에러 코드명** — 현재 `ErrorCode`에 "이미 논리삭제된 리소스를 다시 삭제 시도"에 대응하는 코드가 없어 신규 등록이 필요하다. `CARE_PLAN_SERVICE_ALREADY_DELETED`(409)로 제안하며(기존 네이밍 컨벤션상 `{도메인}_{상태}` 형태를 따름), 확정 시 반영 예정이다.
>
> 참고: 404 에러 코드는 `CARE_PLAN_SERVICE_NOT_FOUND`를 그대로 재사용한다 — 현재 코드에 이미 존재하며 의미도 정확히 일치한다 (`planServiceId`로 `CarePlanService` 조회 실패).
...
// 실패
{
  ...
  "message": "Care Plan 서비스 일정 취소 실패",
  "details": {
    "reason": "planServiceId에 해당하는 확정 서비스 항목이 존재하지 않습니다."
  },
