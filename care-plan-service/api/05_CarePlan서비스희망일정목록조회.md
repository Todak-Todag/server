...
| 2026-09-05 | 구현 완료 — 신규 `ServicePreferenceQueryService.searchServicePreferences()`(조회 책임은 `CarePlanQueryService`가 아닌 이쪽), `ServicePreferenceSearchQuery`/`Result`/`Response`, `ServicePreferenceQueryRepository.search()`(QueryDSL 조인 + `preferredDate` 오름차순), `GET /api/v1/care-plans/{carePlanId}/service-preferences` 엔드포인트(`CarePlanController`) 추가. 이후 리포지토리 반환 타입을 `application/result`가 아닌 `domain/repository/query`의 `ServicePreferenceView`(provider-service `ServiceOfferingView`와 동일 패턴)로 교체해 계층 의존 방향을 정정 |
