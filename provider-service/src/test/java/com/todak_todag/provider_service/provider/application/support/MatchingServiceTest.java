...
class MatchingServiceTest {
    ...
    private final UUID providerIdB = UUID.randomUUID();

    private ServiceOffering offering(UUID id, UUID providerId) {
        ServiceOffering offering = ServiceOffering.of(providerId, UUID.randomUUID(), UUID.randomUUID());
    ...
    private MatchingService.OccupiedSlot slot(UUID providerId, LocalDate date, String startedAt, String finishedAt) {
        return new MatchingService.OccupiedSlot(providerId, date, LocalTime.parse(startedAt), LocalTime.parse(finishedAt));
    }
    ...
    class DayAndTimeSlot {
        ...
        void match_earliestStart() {
            List<ServiceOffering> candidates = List.of(offering(offeringIdA, providerIdA));
            ...
            assertThat(match.get().providerId()).isEqualTo(providerIdA);
        ...
        void match_occupiedRunsPastBusinessHours() {
            ...
            List<MatchingService.OccupiedSlot> occupied = List.of(slot(providerIdA, THURSDAY, "09:00", "23:30"));
    ...
    class OccupiedSlots {
        ...
        void match_afterOccupied() {
            List<ServiceOffering> candidates = List.of(offering(offeringIdA, providerIdA));
            Map<UUID, List<ProvideWork>> works = Map.of(offeringIdA, List.of(work(offeringIdA, 4, "09:00", "13:00")));
            List<MatchingService.OccupiedSlot> occupied = List.of(slot(providerIdA, THURSDAY, "09:00", "10:00"));
        ...
        void match_beforeOccupied() {
            List<ServiceOffering> candidates = List.of(offering(offeringIdA, providerIdA));
            Map<UUID, List<ProvideWork>> works = Map.of(offeringIdA, List.of(work(offeringIdA, 4, "09:00", "13:00")));
            List<MatchingService.OccupiedSlot> occupied = List.of(slot(providerIdA, THURSDAY, "11:00", "12:00"));
        ...
        void match_skipsNarrowGap() {
            List<ServiceOffering> candidates = List.of(offering(offeringIdA, providerIdA));
            Map<UUID, List<ProvideWork>> works = Map.of(offeringIdA, List.of(work(offeringIdA, 4, "09:00", "13:00")));
            List<MatchingService.OccupiedSlot> occupied = List.of(slot(providerIdA, THURSDAY, "09:30", "10:30"));
        ...
        void match_noRoomLeft() {
            List<ServiceOffering> candidates = List.of(offering(offeringIdA, providerIdA));
            Map<UUID, List<ProvideWork>> works = Map.of(offeringIdA, List.of(work(offeringIdA, 4, "09:00", "13:00")));
            List<MatchingService.OccupiedSlot> occupied = List.of(slot(providerIdA, THURSDAY, "09:00", "13:00"));
        ...
        void match_ignoresOtherDate() {
            List<ServiceOffering> candidates = List.of(offering(offeringIdA, providerIdA));
            Map<UUID, List<ProvideWork>> works = Map.of(offeringIdA, List.of(work(offeringIdA, 4, "09:00", "13:00")));
            List<MatchingService.OccupiedSlot> occupied = List.of(slot(providerIdA, FRIDAY, "09:00", "13:00"));
        ...
        void match_ignoresOtherProvider() {
            List<ServiceOffering> candidates = List.of(offering(offeringIdA, providerIdA));
            Map<UUID, List<ProvideWork>> works = Map.of(offeringIdA, List.of(work(offeringIdA, 4, "09:00", "13:00")));
            List<MatchingService.OccupiedSlot> occupied = List.of(slot(providerIdB, THURSDAY, "09:00", "13:00"));
    ...
    class CandidateSelection {
        ...
        void match_prefersLessLoaded() {
            List<ServiceOffering> candidates = List.of(
                    offering(offeringIdA, providerIdA), offering(offeringIdB, providerIdB));
            ...
            List<MatchingService.OccupiedSlot> occupied = List.of(
                    slot(providerIdA, FRIDAY, "09:00", "10:00"),
                    slot(providerIdA, FRIDAY, "10:00", "11:00")
            );
        ...
        void match_prefersEarlierWhenTied() {
            List<ServiceOffering> candidates = List.of(
                    offering(offeringIdB, providerIdB), offering(offeringIdA, providerIdA));
            Map<UUID, List<ProvideWork>> works = Map.of(
                    offeringIdA, List.of(work(offeringIdA, 4, "09:00", "13:00")),
                    offeringIdB, List.of(work(offeringIdB, 4, "10:00", "18:00"))
            );
        ...
        @Test
        @DisplayName("같은 제공자의 다른 제공 서비스 일정도 부하로 센다")
        void match_countsLoadByProvider() {
            // A 제공자는 이 후보(offeringIdA)에는 일정이 없지만 다른 서비스 종류에서 2건이 있다
            List<ServiceOffering> candidates = List.of(
                    offering(offeringIdA, providerIdA), offering(offeringIdB, providerIdB));
            Map<UUID, List<ProvideWork>> works = Map.of(
                    offeringIdA, List.of(work(offeringIdA, 4, "09:00", "13:00")),
            ...
            assertThat(match.get().serviceOfferingId()).isEqualTo(offeringIdB);
            assertThat(match.get().providerId()).isEqualTo(providerIdB);
        }
    }
}